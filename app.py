...
# Try to import ML prediction functions, fallback if not available
try:
...
def create_app():
    ...
    def convert_to_model_input(responses, user):
        """Convert form responses to model input format"""
        # Parse ECG if provided (simple conversion)
        ecg_text = responses.get("ECG", "").lower()
        ecg_ra = 0.0
        ecg_la = 0.0
        ecg_rl = 0.0
        
        if ecg_text:
            # Try to extract numeric values from ECG text
            numbers = re.findall(r'\d+\.?\d*', ecg_text)
            if len(numbers) >= 3:
                ecg_ra = float(numbers[0])
                ecg_la = float(numbers[1])
                ecg_rl = float(numbers[2])
        
        # Map smoking value
        smoker = responses.get("Smoker", "No")
        if smoker == "Yes":
            smoker_mapped = "Yes"
        elif smoker == "Occasionally":
            smoker_mapped = "Occasionally"
        else:
            smoker_mapped = "No"
        ...
        model_input = {
            ...
            "Smoker": smoker_mapped,
            "ActivityLevel": responses.get("ActivityLevel", "Moderate"),
            "Diet": responses.get("Diet", "Average"),
            "Alcohol": responses.get("Alcohol", "Never"),
            "SleepHours": float(responses.get("SleepHours", 0) or 0),
            "FamilyHistory": responses.get("FamilyHistory", "No"),
            "HighBP": responses.get("HighBP", "No"),
            "Diabetes": responses.get("Diabetes", "No"),
            "HeartDisease": responses.get("HeartDisease", "No"),
            "StressLevel": int(responses.get("StressLevel", 0) or 0),
            "Weight": float(responses.get("Weight", 0) or 0),
            "Height": float(responses.get("Height", 0) or 0),
            "BMI": float(responses.get("BMI", 0) or 0),
            "BloodSugar": float(responses.get("BloodSugar", 0) or 0),
            "HeartRate": int(responses.get("HeartRate", 0) or 0),
            "ECG_RA": ecg_ra,
            "ECG_LA": ecg_la,
            "ECG_RL": ecg_rl,
        }
        ...
        if not model_input["BMI"] and model_input["Height"] and model_input["Weight"]:
            h_m = model_input["Height"] / 100.0
            if h_m > 0:
                model_input["BMI"] = round(model_input["Weight"] / (h_m * h_m), 2)
    ...
    def test(userId):
        ...
        if request.method == "POST":
            ...
            # Compute simple risk level
            risk = compute_risk_level(responses)

            # Get ML model prediction
            ml_risk_result = risk  # fallback to simple risk
            ml_confidence = 0.0
            ml_probabilities = {}
            if ML_AVAILABLE:
                try:
                    model_input = convert_to_model_input(responses, user)
                    ml_risk_result, ml_confidence, ml_probs = predict(model_input)
                    # Store probabilities as dict
                    if isinstance(ml_probs, (list, tuple)) and len(ml_probs) >= 3:
                        ml_probabilities = {
                            "Low": float(ml_probs[0]),
                            "Medium": float(ml_probs[1]),
                            "High": float(ml_probs[2])
                        }
                except Exception as e:
                    app.logger.error(f"ML prediction failed: {e}")
                    # Fall back to simple risk calculation
            ...
            test_doc = {
                ...
                "testId": test_id,
                "responses": responses,
                "riskResult": ml_risk_result,  # Use ML prediction as primary
                "simpleRiskResult": risk,  # Keep simple calculation for comparison
                "mlConfidence": ml_confidence,
                "mlProbabilities": ml_probabilities,
                "report": report_meta,
